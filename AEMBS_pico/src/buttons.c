...
#if PL_CONFIG_USE_FROM_ISR_YIELD // SW07!
#include "SW7.h"
#endif
...
static void gpio_IsrCallback(uint gpio, uint32_t events)
{
...
#if PL_CONFIG_USE_FROM_ISR_YIELD // SW07!
  TaskHandle_t sw7TaskHandle = getSw7TaskHandle();
  xHigherPriorityTaskWoken = xTaskResumeFromISR(sw7TaskHandle);
#endif

#if PL_CONFIG_USE_DEBOUNCE // SW08!
  ...
  {
    McuSystemView_Print("HigherPriorityTask awoken! ");
    portYIELD_FROM_ISR(xHigherPriorityTaskWoken);
  }
#endif
